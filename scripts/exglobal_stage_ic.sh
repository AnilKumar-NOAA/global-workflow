...
for MEMDIR in "${MEMDIR_ARRAY[@]}"; do
  ...
  if [[ "${DO_OCN:-}" = "YES" ]]; then
    ...
    err=$((err + rc))
    case "${OCNRES}" in
      "500" | "100")  # 5 degree or 1 degree ocean do not have MOM.res_[1-4].nc files
      ;;
  ...
  if [[ "${DO_ICE:-}" = "YES" ]]; then
    ...
    #src="${BASE_CPLIC}/${CPL_ICEIC}/${PDY}${cyc}/ice/${ICERES}/cice5_model_${ICERESdec}.res_${PDY}${cyc}.nc"
